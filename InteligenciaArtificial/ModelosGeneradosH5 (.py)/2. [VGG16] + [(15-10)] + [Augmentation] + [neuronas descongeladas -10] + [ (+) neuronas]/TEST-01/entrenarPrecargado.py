# - Aumento de datos más robusto (entrenamiento_datagen) 
...
altura, longitud = 224, 224  # Resolución ideal para EfficientNet-B0
batch_size = 32
...
entrenamiento_datagen = ImageDataGenerator(
    rescale=1. / 255,
    shear_range=0.4,  # Incrementa el rango de corte
    zoom_range=0.4,  # Incrementa el rango de zoom
    horizontal_flip=True,
    vertical_flip=True,
    rotation_range=60,  # Más rotaciones
...
# Cargar modelo base preentrenado (EfficientNetB0)
...
for layer in base_model.layers[-10:]:  # Ajusta el número de capas descongeladas (6 capas finales)
...
modelo_guardado = os.path.join(directorio_padre, 'ModelosGeneradosH5', '2. [VGG16] + [(15-15)] + [Augmentation] + [batsize Dynamic] + [neuronas descongeladas -12] + [neuronas]', 'TEST-02 (15-15)','entrenarPrecargado.h5')
...
plt.ylabel('Métrica')
plt.legend()
plt.title('Precisión y Pérdida Durante Entrenamiento')
plt.show()
